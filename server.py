...
def _normalize_plan_value(value: Any, default: str = "free") -> str:
    plan = str(value or default).strip().lower().replace("-", "_").replace(" ", "_")
...
def _payload_plan(payload: dict[str, Any] | None, default: str = "all_access") -> str:
    if not payload:
        return default
    return _normalize_plan_value(payload.get("plan"), default)
...
def _mapping_dict(obj: Any) -> dict[str, Any]:
    if not obj:
        return {}
    if isinstance(obj, dict):
        return obj
...
async def _start_checkout(request: Request, plan: str, user_id: str | None = None):
    normalized = _normalize_plan_value(plan, "pro")
...
async def billing_success(
    ...
    if session_id and settings.stripe_secret_key and store.configured:
        try:
            ...
            user_id = _mapping_value(session, "client_reference_id")
            metadata = _mapping_dict(_mapping_value(session, "metadata", {}))
            synced_plan = _normalize_plan_value(metadata.get("tradepulse_plan") or plan, "pro")
            if user_id:
                store.upsert_subscription(
                    user_id=user_id,
                    plan=synced_plan if synced_plan in PAID_PLANS else "pro",
                    status="active",
                    stripe_customer_id=_mapping_value(session, "customer"),
                    stripe_subscription_id=_mapping_value(session, "subscription"),
                )
...
async def billing_webhook(request: Request, stripe_signature: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    if event_type == "checkout.session.completed":
        ...
        plan = _normalize_plan_value(obj.get("metadata", {}).get("tradepulse_plan"), "pro")
        if user_id and store.configured:
            ...
            subscription_error = "Checkout session did not include a TradePulse user ID."
        elif not store.configured:
            subscription_error = "Supabase service credentials are not configured."
    return {"ok": True, "received": event_type, "subscription_saved": subscription_saved, "subscription_error": subscription_error}
